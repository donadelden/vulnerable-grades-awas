<?php
	...
	if($user!=false && checkAdmin($user)){
		...
		$query = "SELECT exam_date, subject, grade, name, surname, passed, g.username AS usr FROM grades g, users u WHERE g.username = u.username";
		$result = pg_query($conn, $query);
		$error = pg_result_error($result);
		if (pg_num_rows($result) > 0) {
			echo "<table border=\"1px solid\" align=\"center\"><tr><th>Date</th><th>Name</th><th>Surname</th><th>Username</th><th>Subject</th><th>Grade</th><th>Passed</th><th>Delete</th></tr>";
			while ($grade_row = pg_fetch_assoc($result)) {
				if ($grade_row["passed"] == t)
					$passed = "YES";
				else
					$passed = "NO";
